...
// ─── WARNA ───────────────────────────────────────────────────────────────────
const PIE_COLORS_KELUAR = ["#ef4444","#f97316","#eab308","#84cc16","#06b6d4","#8b5cf6"];
const PIE_COLORS_MASUK  = ["#10b981","#3b82f6","#f59e0b","#a855f7","#14b8a6"];
...
function ActiveShape(props: any) {
  ...
  return (
    <g>
      <text x={cx} y={cy - 10} textAnchor="middle" fill="#374151" fontSize={12} fontWeight={600}>{payload.name}</text>
      <text x={cx} y={cy + 10} textAnchor="middle" fill="#6b7280" fontSize={11}>{rupiah(value)}</text>
...
export default function LaporanKeuanganClient() {
  ...
  function handleFilterBulan(v: string) { setBulanFilter(v); setHalaman(1); }
  function handleFilterJenis(v: "semua" | "masuk" | "keluar") { setJenisFilter(v); setHalaman(1); }

  const summaryFiltered = useMemo(() => {
    let m = 0, k = 0;
    for (const t of filtered) { if (t.jenis === "masuk") m += t.jumlah; else k += t.jumlah; }
    ...
    <div className="max-w-5xl mx-auto px-4 py-10 space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Masjid Al-Hidayah · Ketintang, Surabaya · Tahun 2024
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-5 border-l-4 border-emerald-400">
          ...
          <p className="text-2xl font-bold text-emerald-600">{rupiah(summary.masuk)}</p>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-5 border-l-4 border-red-400">
          ...
          <p className="text-2xl font-bold text-red-500">{rupiah(summary.keluar)}</p>
          <p className="text-xs text-gray-400 mt-1">Januari – Desember 2024</p>
        </div>
        {/* Saldo */}
        <div className="bg-white rounded-2xl shadow-sm p-5 border-l-4 border-amber-400">
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Saldo Akhir</p>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6">
        <h2 className="text-[15px] font-bold text-gray-800 mb-4">Pemasukan & Pengeluaran per Bulan</h2>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6">
        <h2 className="text-[15px] font-bold text-gray-800 mb-4">Saldo Kumulatif Bulanan</h2>
        <ResponsiveContainer width="100%" height={220}>
          <LineChart
            data={monthlyData.reduce<{ bulan: string; Saldo: number }[]>((acc, cur, i) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Pie Pengeluaran */}
        <div className="bg-white rounded-2xl shadow-sm p-6">
          <h2 className="text-[15px] font-bold text-gray-800 mb-1">Komposisi Pengeluaran</h2>
          ...
          <ResponsiveContainer width="100%" height={240}>
            <PieChart>
              ...
              >
                {pieKeluar.map((_, i) => <Cell key={i} fill={PIE_COLORS_KELUAR[i % PIE_COLORS_KELUAR.length]} />)}
              </Pie>
            </PieChart>
          </ResponsiveContainer>
          {/* Legenda */}
            ...
            {pieKeluar.map((d, i) => (
              <li key={d.name} className="flex items-center justify-between text-[12px]">
                <span className="flex items-center gap-1.5">
                  <span className="w-2.5 h-2.5 rounded-full inline-block shrink-0" style={{ background: PIE_COLORS_KELUAR[i % PIE_COLORS_KELUAR.length] }} />
                  {d.name}
                </span>
        ...
        {/* Pie Pemasukan */}
        <div className="bg-white rounded-2xl shadow-sm p-6">
          <h2 className="text-[15px] font-bold text-gray-800 mb-1">Komposisi Pemasukan</h2>
          <p className="text-xs text-gray-400 mb-4">Klik slice untuk detail</p>
          <ResponsiveContainer width="100%" height={240}>
            <PieChart>
              <Pie
                ...
                label={renderPieLabel}
              >
                {pieMasuk.map((_, i) => <Cell key={i} fill={PIE_COLORS_MASUK[i % PIE_COLORS_MASUK.length]} />)}
              </Pie>
            </PieChart>
          </ResponsiveContainer>
          <ul className="mt-3 space-y-1">
            {pieMasuk.map((d, i) => (
              <li key={d.name} className="flex items-center justify-between text-[12px]">
                <span className="flex items-center gap-1.5">
                  <span className="w-2.5 h-2.5 rounded-full inline-block shrink-0" style={{ background: PIE_COLORS_MASUK[i % PIE_COLORS_MASUK.length] }} />
                  {d.name}
                </span>
                <span className="font-semibold text-gray-700">{rupiah(d.value)}</span>
              </li>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100">
                <th className="text-left text-[11px] font-semibold text-gray-400 uppercase pb-2 pr-4 whitespace-nowrap">Tanggal</th>
                <th className="text-left text-[11px] font-semibold text-gray-400 uppercase pb-2 pr-4">Keterangan</th>
                <th className="text-left text-[11px] font-semibold text-gray-400 uppercase pb-2 pr-4 whitespace-nowrap">Kategori</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {tabelData.map((t) => (
                <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className={`py-2.5 text-right font-semibold text-[13px] whitespace-nowrap ${
              ...
              {tabelData.length === 0 && (
                <tr><td colSpan={4} className="py-8 text-center text-gray-400 text-sm">Tidak ada data transaksi.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 text-[13px] text-amber-800">
        ...
        <p className="leading-relaxed text-amber-700">
          Laporan keuangan ini disusun oleh Pengurus Masjid Al-Hidayah dan disampaikan secara terbuka kepada jamaah.
          Untuk pertanyaan atau klarifikasi, silakan hubungi pengurus melalui email{" "}
          <a href="mailto:info@masjidalhidayah.id" className="underline font-semibold">info@masjidalhidayah.id</a>.
        </p>
      </div>
