...
export default function JadwalSholatClient({
  ...
  return (
    <div className="relative z-10 -mt-16 px-4 mb-8">
      ...
      >
        {/* ── TOP ROW: location + clock + day toggle ── */}
        <div className="flex items-center justify-between px-5 py-3 gap-4">
          ...
          {/* day tabs — compact pills */}
          <div className="flex bg-white/15 rounded-lg p-0.5 gap-0.5 shrink-0">
            {prayerData.map((r, i) => (
              <button
                key={i}
          ...
          <div className="text-white font-bold text-[15px] tabular-nums shrink-0">
            <LiveClock />
          </div>
        </div>
          ...
          {PRAYERS.map(({ key, label }, idx) => {
            ...
            return (
              ...
              >
                ...
                <p
                  className={`text-[9px] font-bold uppercase tracking-wider ${isNext ? "text-amber-500" : "text-white/70"}`}
                >
                  {label}
                </p>
                <p
                  className={`text-[14px] font-bold tabular-nums leading-none ${isNext ? "text-amber-600" : "text-white"}`}
                >
                  {row[key]}
                </p>
              </div>
        ...
        {/* ── KHATIB STRIP ── */}
        <div
          className={`flex items-center gap-3 px-5 py-2.5 border-t border-white/20 bg-black/10 transition-all duration-700 ${
        ...
        >
          ...
          <div className="flex items-center gap-1.5 min-w-0">
            <span className="text-white/60 text-[10px] font-semibold uppercase tracking-wider shrink-0">
              Khatib Jumat
            </span>
            ...
            {fridayKhatib?.spesialisasi && (
              <>
                ...
                <span className="text-white/60 text-[10px] truncate hidden sm:block">
                  {fridayKhatib.spesialisasi}
                </span>
              </>
            )}
          </div>
          <span className="text-white/50 text-[10px] ml-auto shrink-0 hidden sm:block">
            {khutbahLabel}
          </span>
        </div>
      </div>
