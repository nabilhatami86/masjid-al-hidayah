...
export default function KeuanganAdminPage() {
  ...
  function openAdd() { setForm(EMPTY_FORM); setEditId(null); setError(null); setModalOpen(true); }
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (editId) {
        const updated = await api<TransaksiAdmin>(`/api/transaksi/${editId}`, { method: "PUT", body: JSON.stringify(form) });
        setList(list.map((t) => (t.id === editId ? updated : t)));
      } else {
        const created = await api<TransaksiAdmin>("/api/transaksi", { method: "POST", body: JSON.stringify(form) });
  ...
  function handleJenisChange(j: "masuk" | "keluar") {
    setForm({ ...form, jenis: j, kategori: j === "masuk" ? KATEGORI_MASUK[0] : KATEGORI_KELUAR[0] });
  }
  ...
  const filtered = useMemo(() => list.filter((t) => {
    ...
    return (filterBulan === "semua" || m === filterBulan) &&
           ...
           (search === "" || t.keterangan.toLowerCase().includes(search.toLowerCase()) ||
  ...
  const summary = useMemo(() => {
    ...
    for (const t of filtered) { if (t.jenis === "masuk") masuk += t.jumlah; else keluar += t.jumlah; }
    return { masuk, keluar, saldo: masuk - keluar };
  }, [filtered]);

  const totalSummary = useMemo(() => {
    let masuk = 0, keluar = 0;
    for (const t of list) { if (t.jenis === "masuk") masuk += t.jumlah; else keluar += t.jumlah; }
    return { masuk, keluar, saldo: masuk - keluar };
  }, [list]);
          ...
          <div className="max-w-5xl mx-auto px-5 py-8 space-y-6">
            ...
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-0.5">{loading ? "Memuat…" : `${list.length} transaksi tercatat`}</p>
              </div>
            ...
            <div className="flex flex-wrap gap-2 items-center">
              <div className="relative flex-1 min-w-45">
                ...
                <input type="text" placeholder="Cari keterangan / kategori..." value={search}
              ...
              <select value={filterBulan} onChange={(e) => { setFilterBulan(e.target.value); setHalaman(1); }}
              ...
              <div className="flex rounded-xl overflow-hidden border border-gray-200">
                {(["semua", "masuk", "keluar"] as const).map((j) => (
                  <button key={j} onClick={() => { setFilterJenis(j); setHalaman(1); }}
                    className={`px-3 py-2 text-[12px] font-medium transition-colors capitalize ${
                      filterJenis === j
                        ? j === "masuk" ? "bg-emerald-500 text-white" : j === "keluar" ? "bg-red-500 text-white" : "bg-gray-700 text-white"
            ...
            {(filterBulan !== "semua" || filterJenis !== "semua" || search) && (
              <div className="flex gap-4 text-[13px] bg-white rounded-xl px-4 py-2.5 shadow-sm border border-gray-100">
                <span className="text-emerald-600 font-semibold">Masuk: {rupiah(summary.masuk)}</span>
                <span className="text-red-500 font-semibold">Keluar: {rupiah(summary.keluar)}</span>
                <span className={`font-bold ${summary.saldo >= 0 ? "text-amber-600" : "text-red-600"}`}>Selisih: {rupiah(summary.saldo)}</span>
                <span className="text-gray-400 ml-auto">{filtered.length} transaksi</span>
              </div>
            )}
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    ...
                    {!loading && pageData.length === 0 && <tr><td colSpan={5} className="text-center py-10 text-gray-400">Tidak ada transaksi.</td></tr>}
                    {pageData.map((t) => (
                      <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3 hidden sm:table-cell">
                          <span className="text-[11px] bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full whitespace-nowrap">{t.kategori}</span>
                        </td>
                        <td className={`px-4 py-3 text-right font-bold text-[13px] whitespace-nowrap ${t.jenis === "masuk" ? "text-emerald-600" : "text-red-500"}`}>
                        ...
                        <td className="px-5 py-3 text-right">
                          <div className="flex items-center justify-end gap-1">
                            ...
                            <button onClick={() => setDeleteId(t.id)} className="p-1.5 rounded-lg text-gray-400 hover:bg-red-50 hover:text-red-600 transition-colors">
              ...
              {totalHalaman > 1 && (
                <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => setHalaman((p) => Math.max(1, p - 1))} disabled={safePage === 1}
                      className="px-3 py-1.5 text-[12px] rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 disabled:opacity-40">‹</button>
                    {Array.from({ length: Math.min(totalHalaman, 5) }, (_, i) => {
                      const p = totalHalaman <= 5 ? i + 1 : Math.max(1, Math.min(safePage - 2, totalHalaman - 4)) + i;
                      return (
                    ...
                    <button onClick={() => setHalaman((p) => Math.min(totalHalaman, p + 1))} disabled={safePage === totalHalaman}
                      className="px-3 py-1.5 text-[12px] rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 disabled:opacity-40">›</button>
                  </div>
                </div>
              )}
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            <h2 className="text-[16px] font-bold text-gray-900 mb-5">{editId ? "Edit Transaksi" : "Tambah Transaksi"}</h2>
            {error && <p className="text-red-500 text-[13px] mb-3">{error}</p>}
            <form onSubmit={handleSubmit} className="space-y-3.5">
              {/* Jenis Toggle */}
              <div>
                <label className="block text-[12px] font-semibold text-gray-600 mb-1.5">Jenis Transaksi</label>
                <div className="flex rounded-xl overflow-hidden border border-gray-200">
                  <button type="button" onClick={() => handleJenisChange("masuk")}
                  ...
                  <button type="button" onClick={() => handleJenisChange("keluar")}
              ...
              <div>
                <label className="block text-[12px] font-semibold text-gray-600 mb-1">Kategori <span className="text-red-400">*</span></label>
              ...
              <div>
                <label className="block text-[12px] font-semibold text-gray-600 mb-1">Keterangan <span className="text-red-400">*</span></label>
                <input required type="text" value={form.keterangan}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-[12px] font-semibold text-gray-600 mb-1">Jumlah (Rp) <span className="text-red-400">*</span></label>
              ...
              {form.jumlah > 0 && <p className="text-[12px] text-gray-400 -mt-1">= {rupiah(form.jumlah)}</p>}
