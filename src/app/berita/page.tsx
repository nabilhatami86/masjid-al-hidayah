...
import { getAllBerita, getBeritaColors, formatTanggalBerita } from "@/lib/controllers/beritaController";

export const metadata = {
  title: "Berita & Pengumuman | Masjid Al-Hidayah",
  description: "Informasi terkini seputar kegiatan dan pengumuman Masjid Al-Hidayah Ketintang Surabaya.",
};

export const dynamic = "force-dynamic";

export default async function BeritaListPage() {
  const beritaList = await getAllBerita().catch(() => []);
    ...
    <div className="relative min-h-screen overflow-hidden bg-[#F1ECE3]">
      ...
      <main className="relative pt-[97px]">
        <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 sm:py-14">
          ...
          >
            <ArrowLeft size={15} strokeWidth={2.5} className="transition-transform group-hover:-translate-x-0.5" />
            Kembali ke Beranda
          </Link>
            ...
            <div className="flex flex-wrap items-end justify-between gap-4">
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm leading-relaxed text-gray-500 sm:text-[15px]">
                  Update agenda, program, dan pengumuman penting Masjid Al-Hidayah untuk jamaah.
                </p>
              </div>
          ...
          {beritaList.length === 0 ? (
            <div className="text-center py-16 text-gray-400 text-sm bg-white/70 rounded-3xl">
                ...
                return (
                  ...
                  >
                    <div className="mb-4 flex items-center justify-between gap-3">
                      <span className={`inline-flex items-center rounded-full border border-white/80 bg-white/90 px-3 py-1 text-[10.5px] font-bold uppercase tracking-widest ${fc.labelColor}`}>
                        <span className={`mr-1.5 inline-block h-1.5 w-1.5 rounded-sm ${fc.dotColor}`} />
                        {featured.kategori}
                      </span>
                      <span className="flex items-center gap-1 text-[11px] text-gray-400">
                        <CalendarDays size={12} strokeWidth={1.8} />
                        {formatTanggalBerita(featured.tanggal)}
                      </span>
                    </div>
                    ...
                    <div className="mt-5 inline-flex items-center gap-2 text-[12px] font-semibold uppercase tracking-wider text-amber-700">
                      Baca Selengkapnya
                      <ChevronRight size={16} strokeWidth={2.3} className="transition-transform group-hover:translate-x-0.5" />
                    </div>
                  </Link>
                );
              })()}
                  ...
                  return (
                    ...
                    >
                      <div className="mb-3 flex items-center justify-between gap-3">
                        <span className={`text-[10.5px] font-bold uppercase tracking-widest ${bc.labelColor}`}>
                          <span className={`mr-1.5 inline-block h-1.5 w-1.5 rounded-sm ${bc.dotColor}`} />
                          {b.kategori}
                        </span>
                        <span className="flex items-center gap-1 text-[11px] text-gray-400">
                          <CalendarDays size={12} strokeWidth={1.8} />
                          {formatTanggalBerita(b.tanggal)}
                        </span>
                      </div>
                      <h3 className="mb-1.5 text-[15px] font-bold leading-snug text-gray-900 transition-colors group-hover:text-amber-800">
                        {b.judul}
                      </h3>
                      <p className="line-clamp-2 text-[12.5px] leading-relaxed text-gray-500">{b.ringkasan}</p>
